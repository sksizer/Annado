...
export const createTaskSlice: SliceCreator<TaskSlice> = (set, get) => {
  ...
  deleteMultipleTasks: async (ids) => {
    ...
    try {
      // Capture a snapshot per deleted task so the whole batch is reversible
      // with a single ⌘Z (restore_task re-inserts each at its original position).
      const snapshots = await Promise.all(
        [...idSet].map((id) => invoke<DeletedTaskSnapshot>('delete_task', { id }))
      );
      pushUndo(async () => {
        await Promise.all(snapshots.map((s) => get().restoreTask(s)));
      });
    } catch (error) {
